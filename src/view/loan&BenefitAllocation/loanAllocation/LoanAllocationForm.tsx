...
const LoanAllocationForm = ({
    ...
    const validationSchema = Yup.object().shape({
        ...
        interestRate: Yup.string().when("isInterestRateApplicable", {
            is: (isInterestRateApplicable: boolean) => isInterestRateApplicable,
            then: schema => schema.required(commonError).min(0, "Invalid input"),
            otherwise: schema => schema.nullable()
        }),
        isLoanLimitApplicable: Yup.boolean(),
        isLimitValidityPeriodApplicable: Yup.boolean(),
        limitValidityPeriod: Yup.string().when("isLimitValidityPeriodApplicable", {
            is: (isLimitValidityPeriodApplicable: boolean) => isLimitValidityPeriodApplicable,
            then: schema => schema.required(commonError).min(0, "Invalid input"),
            otherwise: schema => schema.nullable()
        }),
        isInterestRateApplicable: Yup.boolean(),
        isInterestRateValidityPeriodApplicable: Yup.boolean(),
        interestRateValidity: Yup.string().when("isInterestRateValidityPeriodApplicable", {
            is: (isInterestRateValidityPeriodApplicable: boolean) => isInterestRateValidityPeriodApplicable,
            then: schema => schema.required(commonError).min(0, "Invalid input"),
            otherwise: schema => schema.nullable()
        }),
        interestRateValidityPeriod: Yup.string().when("isInterestRateValidityPeriodApplicable", {
            is: (isInterestRateValidityPeriodApplicable: boolean) => isInterestRateValidityPeriodApplicable,
            then: schema => schema.required(commonError).min(0, "Invalid input"),
            otherwise: schema => schema.nullable()
        }),
        ...
        loanAmount: Yup.string().when("isLoanLimitApplicable", {
        ...
        try {
            ...
            if (res?.data?.statusCode === 200) {
                ...
                const values = {
                    ...
                    isInterestRateValidityPeriodApplicable: data?.isInterestRateValidityPeriodApplicable,
                    interestRateValidity: data?.isInterestRateValidityDays && !data?.isInterestRateValidityMonth? "daily" : !data?.isInterestRateValidityDays && data?.isInterestRateValidityMonth? "monthly" : "",
                    interestRateValidityPeriod: data?.interestRateValidityPeriod,
                    limitValidity: data?.isLimitValidityMonth && !data?.isLimitValidityDays ? "monthly" : !data?.isLimitValidityMonth && data?.isLimitValidityDays? "daily" : "",
    ...
    const isLimitValidityPeriodApplicableW = watch("isLimitValidityPeriodApplicable");

    const isInterestRateApplicableW = watch("isInterestRateApplicable");

    const isInterestRateValidityPeriodApplicableW = watch("isInterestRateValidityPeriodApplicable");
            ...
            <Box display={"flex"} flexDirection={"column"} gap={2}>
                ...
                <Box display={"flex"} gap={2}>
                    ...
                    <FormAutoCompleteField
                        ...
                        helperText={errors?.limitValidity?.message ? errors?.limitValidity?.message?.toString() : ""}
                        id={"limitValidity"}
                        required={isLimitValidityPeriodApplicableW? isLimitValidityPeriodApplicableW : false}
                        control={control}
                        setValue={setValue}
                        disabled={isView} watch={watch}/>
                </Box>
                <Grid container>
                    <Grid item md={6}>
                        <FormTextField
                ...
                <Box display={"flex"} gap={2}>
                    <FormSwitchField
                        ...
                        setValue={() => setValue}/>
                    <FormTextField
                        required={isInterestRateApplicableW? isInterestRateApplicableW : false}
                        label={"Interest Rate"}
                ...
                <Box display={"flex"} gap={2}>
                    ...
                    <FormAutoCompleteField
                        ...
                        helperText={errors?.interestRateValidity?.message ? errors?.interestRateValidity?.message?.toString() : ""}
                        id={"interestRateValidity"}
                        required={isInterestRateValidityPeriodApplicableW? isInterestRateValidityPeriodApplicableW : false}
                        control={control}
                        setValue={setValue}
                        disabled={isView} watch={watch}/>
                </Box>
