...
import MainBenefitLimit from "./MainBenefitLimit";

interface BenefitAllocationProps {
    setLoading: (loading: boolean) => void;
    categoryId: any;
    categoryData: Array<any>;
}

const BenefitAllocation = ({setLoading, categoryId, categoryData}: BenefitAllocationProps) => {
    ...
    const {fields, remove, prepend} = useFieldArray({control, name: "mainBenefitLimits"});
    ...
    const getMainBenefits = async () => {
        try {
            ...
            if (res?.data?.statusCode === 200) {
                const activeMainBenfits = res?.data?.data?.filter((d: any) => d?.status);
                setMainBenefitOptions(activeMainBenfits?.map((d: any) => ({
    ...
    const handleAddMainCat = () => {
        if (!!mainBenefit) {
            const dupMainBenefit = fields?.find((b: any) => b?.mainBenefitId === mainBenefit);
            if (!dupMainBenefit) {
                prepend({
                    mainBenefitId: mainBenefit,
                    mainBenefitName: mainBenefits?.find((b: any) => b.mainBenefitId === mainBenefit)?.mainBenefitName,
                    mainBenefitCode: mainBenefits?.find((b: any) => b.mainBenefitId === mainBenefit)?.mainBenefitCode,
                    maxAllowedLimit: ""
                });
            ...
            try {
                const payload = {
                    ...
                    mainBenefitLimits: getValues()?.mainBenefitLimits?.map((f: any) => ({
                        ...
                        customerCategoryId: categoryId,
                        subBenefitLimits: f?.subBenefitLimits?.map((l: any) => ({
                            maxAllowedQuantity: l?.maxAllowedLimit,
                            subBenefitId: l?.subBenefitId
                        }))?? []
                    })),
                }
                const res = havaData? await axiosInstance.put(`/benefit-limits/update-benefit-limit`, payload) : await axiosInstance.post(`/benefit-limits/create-benefit-limit`, payload);
                if (res?.data?.statusCode === 200) {
    ...
    return (
        <Box sx={{p: 5}}>
            <Grid container spacing={2}>
                ...
                <Grid item md={2}>
                    <Button variant={"outlined"} size={"small"} onClick={handleAddMainCat}>Add</Button>
                </Grid>
            </Grid>
            ...
            <Box display={"flex"} gap={2} sx={{mt: 5}}>
                <Button variant={"outlined"} size={"small"} onClick={clearData}>Clear</Button>
